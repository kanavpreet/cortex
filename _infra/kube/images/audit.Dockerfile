# Production Dockerfile for the Root Cause Coverage Audit
...
# Install dependencies with audit extras only
RUN uv sync --frozen --no-install-project --no-dev --extra audit
...
# Copy only the service and shared code. enigmatologist is included because
# braintrust_client.py reuses its parse_llm_correlations (the same output
# shape production correlation traces already use).
COPY --chown=matik:matik matik/audit /app/audit
...
# Run the audit application with dumb-init for proper signal handling
ENTRYPOINT ["dumb-init", "--"]
CMD ["python", "-m", "audit.root_cause_coverage"]
