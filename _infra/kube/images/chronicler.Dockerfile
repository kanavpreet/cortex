# Production Dockerfile for Chronicler Service
...
# Install dependencies with chronicler extras only
RUN uv sync --frozen --no-install-project --no-dev --extra chronicler

# Copy source code
COPY matik/ .

# Install the project
RUN uv sync --frozen --no-dev --extra chronicler
...
COPY --chown=matik:matik matik/chronicler /app/chronicler
