# Production Dockerfile for Scribe Service
...
# Install dependencies with scribe extras only
RUN uv sync --frozen --no-install-project --no-dev --extra scribe

# Copy source code
COPY matik/ .

# Install the project
RUN uv sync --frozen --no-dev --extra scribe
...
# This ensures we can validate RDS certificates when connecting through VPC endpoints
...
COPY --chown=matik:matik matik/scribe /app/scribe
