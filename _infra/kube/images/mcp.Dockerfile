# Production Dockerfile for Historian Service
...
# Install dependencies with mcp_server extras only
RUN uv sync --frozen --no-install-project --no-dev --extra mcp_server

# Copy source code
COPY matik/ .

# Install the project
RUN uv sync --frozen --no-dev --extra mcp_server
...
COPY --chown=matik:matik matik/mcp_server /app/mcp_server
